...
def parse_json_field(value) -> list:
    """
    asyncpg returns json_agg() columns as raw strings rather than parsed
    Python objects. This ensures any json/jsonb column is always returned
    as a proper Python list or dict, never a string.
    """
    if value is None:
        return []
    if isinstance(value, str):
        return json.loads(value)
    return value  # already deserialized (jsonb columns)


def normalize_primer_row(row: dict, is_private: bool = True) -> dict:
    """
    Convert a raw asyncpg row dict into a clean, frontend-ready dict:
      - forward_primers  → list of {name, sequence} objects
      - reverse_primers  → list of {name, sequence} objects
      - references       → list of {doi, year, title} objects
    Also strips private fields when the caller is unauthenticated.
    """
    row["forward_primers"] = parse_json_field(row.get("forward_primers")) or []
    row["reverse_primers"] = parse_json_field(row.get("reverse_primers")) or []
    row["references"]      = parse_json_field(row.get("references"))      or []
    if not is_private:
        row.pop("hervolution", None)
    return row
...
async def get_primer_sets(
    ...
    query = """
        SELECT
            ...
            json_agg(DISTINCT CASE WHEN p.direction='forward' THEN jsonb_build_object(
                'name', p.name, 'sequence', p.sequence
            ) END) FILTER (WHERE p.direction='forward') AS forward_primers,
            json_agg(DISTINCT CASE WHEN p.direction='reverse' THEN jsonb_build_object(
                'name', p.name, 'sequence', p.sequence
            ) END) FILTER (WHERE p.direction='reverse') AS reverse_primers,
            COALESCE(
                json_agg(DISTINCT jsonb_build_object(
                    'title', b.title, 'doi', b.doi, 'year', b.year
    ...
    if family:
        query += f" AND f.name = ${idx}"; params.append(family); idx += 1
    if subgroup:
        query += f" AND s.name = ${idx}"; params.append(subgroup); idx += 1
    if component:
        query += f" AND c.name = ${idx}"; params.append(component); idx += 1
    if herv_name:
        query += f" AND (f.name || COALESCE('_' || s.name, '') || '_' || c.name) = ${idx}"
        params.append(herv_name); idx += 1
    if dna is not None:
        query += f" AND ps.dna = ${idx}"; params.append(dna); idx += 1
    if hervolution is not None:
        query += f" AND ps.hervolution = ${idx}"; params.append(hervolution); idx += 1

    query += " GROUP BY ps.id, ps.set_index, ps.dna, ps.hervolution, f.name, s.name, c.name ORDER BY ps.set_index;"

    async with app.state.db.acquire() as conn:
        rows = await conn.fetch(query, *params)

    return [normalize_primer_row(dict(row), is_private) for row in rows]
...
async def primer_loci(
    ...
    if set_index:
        query += f" AND ps.set_index = ${idx}"; params.append(set_index); idx += 1
    if genome_build:
        query += f" AND lc.genome_build = ${idx}"; params.append(genome_build)
...
async def primers_forward(forward_seq: str):
    """
    Given a forward primer sequence, return all reverse primers associated
    with the same biological targets, including references.
    """
    ...
    query = """
        SELECT
            ...
            c.name                          AS component_name,
            json_agg(DISTINCT CASE WHEN p_rev.direction='reverse' THEN jsonb_build_object(
                'name', p_rev.name, 'sequence', p_rev.sequence
            ) END) FILTER (WHERE p_rev.direction='reverse') AS reverse_primers,
            COALESCE(
                json_agg(DISTINCT jsonb_build_object(
                    'title', b.title, 'doi', b.doi, 'year', b.year
                )) FILTER (WHERE b.id IS NOT NULL),
    ...
    return [normalize_primer_row(dict(row)) for row in rows]


@app.get("/primers_reverse")
async def primers_reverse(reverse_seq: str):
    """
    Given a reverse primer sequence, return all forward primers associated
    with the same biological targets, including references.
    """
    ...
    query = """
        SELECT
            ...
            COALESCE(s.name, 'Unspecified') AS subgroup_name,
            c.name                          AS component_name,
            json_agg(DISTINCT CASE WHEN p_fwd.direction='forward' THEN jsonb_build_object(
                'name', p_fwd.name, 'sequence', p_fwd.sequence
            ) END) FILTER (WHERE p_fwd.direction='forward') AS forward_primers,
            COALESCE(
                json_agg(DISTINCT jsonb_build_object(
                    'title', b.title, 'doi', b.doi, 'year', b.year
                )) FILTER (WHERE b.id IS NOT NULL),
...
async def get_primers(
    ...
    query = """
        SELECT
            ...
            c.name                          AS component_name,
            json_agg(DISTINCT jsonb_build_object(
                'name', p_forward.name, 'sequence', p_forward.sequence
    ...
    if forward_seq:
        ...
        params.append(forward_seq); idx += 1
    if reverse_seq:
        query += f" AND UPPER(REPLACE(p_rev.sequence, ' ', '')) = ${idx}"
        params.append(reverse_seq); idx += 1
    if family:
        query += f" AND f.name = ${idx}"; params.append(family); idx += 1
    if subgroup:
        query += f" AND s.name = ${idx}"; params.append(subgroup); idx += 1
    ...
    query += " GROUP BY ps.set_index, ps.dna, f.name, s.name, c.name ORDER BY ps.set_index;"
